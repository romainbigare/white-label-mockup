...
The bar reads **mockup v1.7.0 · spec v1.7**, and the two numbers answer
...
the harness bar prints both — `mockup v1.7.0 · spec v1.7`. `MOCKUP_VERSION` is
...
the reason moved into its registry note, which is the deck's speaker note for
that page.
...
One comment is answered in the deck's speaker note rather than in the app, and
it is in the table below.

[`docs/Mockup_Changes_v157.md`](docs/Mockup_Changes_v157.md) is the round after
those, and the reason the number moved again: the app dropping "white label"
from its name, four new languages and a language screen that no longer scrolls,
one type size across the six tour panels with their screenshots whole rather
...
| 1 Sep, A9B | **Should the fork come before the farm's details?** *"A9B should come before A9 … A9 only applies if user selects 'Survey my whole farm'."* | **Open, and said out loud in the deck.** The second pass asked for the slides to keep the app's order "and a visible note to the powerpoint explaining it", which A9B's page carried as a green card in its right-hand column until the comments on the v1.5.6 deck asked for the card to come off — macOS PowerPoint was drawing it onto the slides after it. The words are in A9B's registry note, which is the speaker note for that page. The reasoning: A9's *what is growing on this land* is what decides whether the fork appears at all: a farm with any trees on it never sees A9B, because trees are counted from imagery and cannot be traced by hand — settled at the 22 August review. Asking the fork first would offer a date grower a route ending in his being told he cannot take it. There is a reading that works — the fork, then the farm's details only on the survey route — but it moves the farm's name and needs a decision about what a tree grower sees. The pages are printed in the app's order and the note is in the speaker notes; the app is unchanged pending that call. |
