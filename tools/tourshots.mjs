...
/* ---------------------------------------------------------------------------
   ...
   THE WHOLE SCREEN, AND NO STATUS BAR. Review 08/09: "keep the whole image and
   make it skinnier rather than cropping — but do not increase the height", and
   "remove the indicator/time element at the top right".

   The first version cropped to the top 62%, on the argument that a whole 852 px
   phone squeezed into a third of a tour panel is a picture of nothing. The
   review disagreed, and the arithmetic is on its side: two pictures share the
   panel's width, so keeping the whole phone makes each NARROWER rather than
   making the panel taller — the tour still fits WF2.002's 360 × 640, and the
   reviewer sees a screen rather than a slice of one.

   What does come off is the harness's own chrome: the fake status bar with the
   clock and the network indicator, the camera cutout inside it, and the home
   bar at the foot. None of that is the app, and a picture of a picture of a
   phone is exactly the thing the reviewer was pointing at. The clip starts
...
await page.addStyleTag({ content: `
  ...
  /* The reviewer's "indicator/time element", and the two pieces of handset
     furniture that go with it. The picture is of the app, not of a phone. */
  .device__statusbar, .device__cutout, .device__homebar { display: none !important; }
` });
...
for (const shot of SHOTS) {
  ...
  const clip = await page.evaluate(() => {
    ...
    // The app reserves --safe-top for the bar that has just been hidden, so the
    // clip starts below it — otherwise every picture opens on a blank strip
    // where the clock used to be.
    const top = parseFloat(getComputedStyle(document.getElementById('app')).getPropertyValue('--safe-top')) || 0;
    return { x: r.x, y: r.y + top, width: r.width, height: r.height - top };
  });
